...
import java.util.HashSet;
         ...
         *
         *
         * This is a placeholder class for the ShowSearcherBackend object
         *
         */
public class Surya_ShowSearcherBackendPlaceholder implements IShowSearcherBackend
{
        private HashMap<String,List<IShow>> set1; //This HashMao maps a keyword to TV titles that include the keyword
        private HashMap<Integer,List<IShow>> set2; //This HashMap maps a year to TV titles that were produced in that year
         ...
         *
         */
	public Surya_ShowSearcherBackendPlaceholder()
        {
                set1 = new HashMap<String, List<IShow>>();
                set2 = new HashMap<Integer,List<IShow>>();
	...
	/**
         *
         *
         * Takes the show title and takes every keyword and maps it to the show, and does the sam>
         *
         *
         */
		...
		if(size == 0)
		{
		 	List<IShow> temp = new ArrayList<IShow>();
			temp.add(show);
			for(String k: split)
                	{
                        	set1.put(k, temp);
                	}
			set2.put(show.getYear(), temp);
		}
                else
		{
			for(String k: split)
			...
			if(set2.containsKey(show.getYear()))
			{
				List<IShow> need2 = set2.get(show.getYear());
				need2.add(show);
				set2.put(show.getYear(), need2);
			}
			else
			{
				List<IShow> temp = new ArrayList<IShow>();
	...
	public void setProviderFilter(String provider, boolean filter )
		...
		titles.clear(); //Clears the original list
		titles.addAll(filtered); //Adds all the elements from the filtered list to the original, since a set cannot be sorted
                Collections.sort(titles); //Sorts in descending order
		return titles;
        }
        /**
		...
		titles.clear(); //Clears the original list
                titles.addAll(filtered); //Adds all the elements from the filtered list to the original list
                Collections.sort(titles); //Sorts in descending order
                return titles;
        }
}
